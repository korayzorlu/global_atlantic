...
def commericalInvoicePdf(theRequest, orderConfirmation, sourceCompany):
    ...
    folderPath = os.path.join(os.getcwd(), "media", "docs", str(sourceCompany.id), "sale", "order_tracking", "documents", theRequest.requestNo)
    ...
    #standart ayar
    p = canvas.Canvas(folderPath + "/" + orderConfirmation.orderConfirmationNo + "-CI.pdf", pagesize = A4)
    ...
    p.drawCentredString(w/2, h-120, "COMMERICAL INVOICE & DELIVERY NOTE")
    ...
    for i in range(0, len(parts), slice):
        ...
        #####sağ üst yazılar#####
        p.setFont('Inter-Bold', 7)
        p.drawString(420, h-50, "PROJECT NO")
        p.setFont('Inter', 7)
        p.drawString(480, h-50, ":" + str(theRequest.requestNo))
        #####sağ üst yazılar-end#####
        ...
        p.setFont('Inter', 7)
        data=[("")]
        table = Table(data, colWidths=w-60, rowHeights=30)
        table.setStyle(TableStyle([
                                    ('GRID', (0,0), (-1,-1), 0.25, colors.grey),
                                ]))
