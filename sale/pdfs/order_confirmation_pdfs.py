...
def orderConfirmationPdf(theRequest, orderConfirmation, sourceCompany):
    ...
    theRequest = Request.objects.get(project = orderConfirmation.project.id)
    #parts = quotation.quotationpart_set.select_related("inquiryPart")
    parts = QuotationPart.objects.filter(quotation = orderConfirmation.quotation).order_by("sequency", "alternative")
    extras = QuotationExtra.objects.filter(quotation = orderConfirmation.quotation)
    items = []
    for part in parts:
        items.append({"id" : part.id,
                    "name":part.inquiryPart.requestPart.part.partNo,
                      ...
                      "currency" : orderConfirmation.quotation.currency.code,
    ...
    folderPath = os.path.join(os.getcwd(), "media", "docs", str(sourceCompany.id), "sale", "order_confirmation", "documents")
    ...
    p.drawCentredString(w/2, h-100, "ORDER CONFIRMATION")
    ...
    if theRequest.vessel:
        ...
        equipment = EnginePart.objects.filter(vessel = vessel, maker = theRequest.maker, makerType = theRequest.makerType).first()
        if equipment:
    ...
    data=[[orderConfirmation.quotation.quotationNo],
    ...
    for key, theItems in enumerate(allItems):
        ...
        p.setFont('Inter', 7)
        p.drawString(480, h-50, ":" + str(orderConfirmation.orderConfirmationDate.strftime("%d.%m.%Y")))
        #####sağ üst yazılar-end#####
        ...
        #####parts tablo#####
        data=[["Line", "Item", "Description", "Notes", "Qty", "Unit", "Unit Price", "Total Price"]]
        ...
        for j in range(len(theItems["parts"])):
            ...
            data.append([sequency, theItems["parts"][j]["name"], newDescription, newNote, theItems["parts"][j]["quantity"], theItems["parts"][j]["unit"], str(unitPrice3Fixed) + " " + str(theItems["parts"][j]["currency"]), str(totalPrice3Fixed) + " " + str(theItems["parts"][j]["currency"])])
        
        table = Table(data, colWidths=[((w-60)/100)*4 , ((w-60)/100)*15, ((w-60)/100)*40, ((w-60)/100)*9, ((w-60)/100)*4 , ((w-60)/100)*4 , ((w-60)/100)*12 , ((w-60)/100)*12])
        ...
        p.setFont('Inter-Bold', 7)
        p.drawString(30, 30, str(orderConfirmation.project.user.first_name) + " " + str(orderConfirmation.project.user.last_name) + " / " + str(orderConfirmation.project.user.profile.positionType))
        ...
        if key == len(allItems) - 1 and extraPage == 0:
            ...
            totalTable.wrapOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4), th-215-sum(table._rowHeights)-52)
            totalTable.drawOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4), th-215-sum(table._rowHeights)-52)
            ...
            totalTable.wrapOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4)+tableTotalLeftLength, th-215-sum(table._rowHeights)-52)
            totalTable.drawOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4)+tableTotalLeftLength, th-215-sum(table._rowHeights)-52)
            tableTotalLeftLength = totalTable._colWidths[0]
            ...
            if orderConfirmation.note:
                p.drawString(30, th-225-sum(table._rowHeights), "NOTES")
                p.setFont('Inter', 7)
            ...
            lines = lines.split('\n')
            line_height = 10  # İsteğe bağlı, satır yüksekliği
            current_y = th-225-sum(table._rowHeights)
        ...
        if key == len(allItems) - 1 and extraPage == 1:
            ...
            totalTable.wrapOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4), th-215-52)
            totalTable.drawOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4), th-215-52)
            tableTotalLeftLength = totalTable._colWidths[0]
            ...
            totalTable.wrapOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4)+tableTotalLeftLength, th-215-52)
            totalTable.drawOn(p, 30+(((w-60)/100)*4)+(((w-60)/100)*15)+(((w-60)/100)*40)+(((w-60)/100)*9)+(((w-60)/100)*4)+(((w-60)/100)*4)+tableTotalLeftLength, th-215-52)
            tableTotalLeftLength = totalTable._colWidths[0]
            ...
            p.drawString(450, h-50, "DATE")
            p.setFont('Inter', 7)
            p.drawString(480, h-50, ":" + str(orderConfirmation.quotation.quotationDate.strftime("%d.%m.%Y")))
            #####sağ üst yazılar-end#####
