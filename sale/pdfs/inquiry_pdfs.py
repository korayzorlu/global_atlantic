...
from ..models import InquiryPart, Inquiry
...
def inquiryPdf(id,sourceCompanyId):
    ...
    inquiry = Inquiry.objects.select_related("theRequest").filter(id = id).first()
    theRequest = inquiry.theRequest
    parts = inquiry.inquirypart_set.select_related("requestPart__part").order_by("sequency").all()
    items = []
    for part in parts:
        if part.quantity == 0:
            quantity = int(0)
        ...
        items.append({"id" : part.id,
                      "name":part.requestPart.part.partNo,
                      "description" : part.requestPart.part.description,
                      "remark" : part.remark,
                      "quantity" : quantity,
                      "unit" : part.requestPart.part.unit,
                      "note" : part.note,
    ...
    folderPath = os.path.join(os.getcwd(), "media", "docs", str(sourceCompany.id), "sale", "inquiry", "documents")
    ...
    for key, theItems in enumerate(allItems):
        ...
        for j in range(len(theItems["parts"])):
            ...
            data.append([sequency, theItems["parts"][j]["name"], newDescription, newNote, theItems["parts"][j]["quantity"], theItems["parts"][j]["unit"]])
