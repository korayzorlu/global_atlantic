...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        data = pd.read_excel("./excelfile/parts-data-multiple.xlsx", "MITSUIMAN S35MC")
        ...
        for i in range(len(df["MAKER"])):
            ...
            if pd.isnull(df["CROSS REF"][i]):
                crossRef = ""
            else:
                crossRef = df["CROSS REF"][i]
                
            if pd.isnull(df["DRAWING NO"][i]):
                drawingNr = ""
            else:
                drawingNr = df["DRAWING NO"][i]
