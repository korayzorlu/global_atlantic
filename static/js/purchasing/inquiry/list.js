...
function setPurchasingInquiryDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/purchasing/inquiry_add/";
...
/**/let deleteDataUrl = "/purchasing/inquiry_delete/";
    let serverSide = true;
/**/let apiSource = '/purchasing/api/inquiries?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "inquiryNo", className:"double-clickable ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/purchasing/inquiry_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        if(stage == "project" || stage == "inquiry"){
          $('td:eq(8)', row).addClass("tableAlert-secondary");
  ...
  /**/htmx.ajax('GET', '/purchasing/inquiry_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/purchasing/inquiry_update/' + data["id"] + '/');
    });
...
function setNavTabPurchasingInquiry(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
...
$(document).ready(function () {
  /**/setPurchasingInquiryDataTable();
  setNavTabPurchasingInquiry();
