...
function setProjectDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/purchasing/project_add/";
...
/**/let deleteDataUrl = "/purchasing/project_delete/";
    let serverSide = true;
/**/let apiSource = '/purchasing/api/projects?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "projectNo", className:"double-clickable ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/purchasing/project_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "projectDate", className:"double-clickable ps-1 pe-1"},
                    ...
                    {"data" : "stage", className:"double-clickable shadow-0 ps-1 pe-1", "width" : "9%", render: function (data, type, row, meta){
                      if(data == "project"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="secondary" style="padding:2px 5px;margin:0;"><i class="fas fa-sheet-plastic me-3"></i>Project</div>';
                        ...
                        return '<div class="alert text-start" role="alert" data-mdb-color="warning" style="padding:2px 5px;margin:0;"><i class="fas fa-file-invoice-dollar me-3"></i>Warehouse</div>';
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        if(stage == "project" || stage == "inquiry"){
          $('td:eq(7)', row).addClass("tableAlert-secondary");
        }else if(stage == "purchase_order"){
          $('td:eq(7)', row).addClass("tableAlert-primary");
  ...
  /**/htmx.ajax('GET', '/purchasing/project_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/purchasing/project_update/' + data["id"] + '/');
    });
