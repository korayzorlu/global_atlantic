function setPurchasingPurchaseOrderItemDetailDatatable(){
...
/**/let addDataHxGet = "/sale/quotation_part_update/" + elementTagId + "/";
    ...
    let editor = new $.fn.dataTable.Editor({
        ajax: "/purchasing/api/purchase_order_items/editor/",
        table: tableId,
        idSrc: "id",
        fields: [
            {
...
/**/let deleteDataUrl = "/purchasing/purchase_order_item_delete/";
    let serverSide = false;
/**/let apiSource = '/purchasing/api/purchase_order_items?purchaseOrder=' + purchaseOrderId + '&format=datatables';
/**/let columns = [
        {
            orderable: false,
        ...
        {"data" : "name", className:"text-start ps-1 pe-1", "width": "15%"},
        ...
        {"data" : "placed","visible":false},
        {"data" : "",width:"10%", render: function (data, type, row, meta){
            if(row.placed){
                return '<div class="alert text-start" role="alert" data-mdb-color="success" style="padding:2px 0px;margin:0;"><i class="fas fa-circle-check me-2"></i>Placed</div>'
            }else{
                return `<button class="button-30 placePart" type="button" hx-get="/purchasing/purchase_order_item_place?iii=${row.id}" hx-target="#addUpdateDataDialogXl"">
                            Place <i class="fa-solid fa-dolly"></i>
                        </button>`
            };
...
function formSubmitMessagePurchasingPurchaseOrder(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
...
$(document).ready(function () {
    setPurchasingPurchaseOrderItemDetailDatatable();
    //setHTMX();
    setNavTabSubPurchasingPurchaseOrder();
    formSubmitMessagePurchasingPurchaseOrder();
