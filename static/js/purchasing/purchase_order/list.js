...
function setPurchasingPurchaseOrderDataTable(){/**/
...
/**/let deleteDataUrl = "/purchasing/purchase_order_delete/";
    let serverSide = true;
/**/let apiSource = '/purchasing/api/purchase_orders?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "purchaseOrderNo", className:"double-clickable ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/purchasing/purchase_order_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    ...
                    {"data" : "projectStage", className:"double-clickable shadow-0 ps-1 pe-1", "width" : "9%", render: function (data, type, row, meta){
                      if(data == "project"){
                        ...
                        return '<div class="alert text-start" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-circle-check me-3"></i>Invoiced</div>';
                      };
                    }
                    }
    ];
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        var stage = data.projectStage;
        if(stage == "project" || stage == "inquiry"){
          $('td:eq(10)', row).addClass("tableAlert-secondary");
        }else if(stage == "purchase_order"){
          $('td:eq(10)', row).addClass("tableAlert-primary");
        }else if(stage == "warehouse"){
          $('td:eq(10)', row).addClass("tableAlert-warning");
        }else if(stage == "invoiced"){
          $('td:eq(10)', row).addClass("tableAlert-success");
        };
      ...
      drawCallback: function() {
        ...
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
    ...
    /**/htmx.ajax('GET', '/purchasing/purchase_order_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/purchasing/purchase_order_update/' + data["id"] + '/');
    });
...
function setNavTabPurchasingPurchaseOrder(){
  ...
  function setNavTabSubPurchasingPurchaseOrder(){
...
function setHTMXPurchasingPurchaseOrder(){
  let ee = elementTag;
  let ei = elementTagId;
...
/**/setPurchasingPurchaseOrderDataTable();
    setNavTabPurchasingPurchaseOrder();
    setNavTabSubPurchasingPurchaseOrder();
    setHTMXPurchasingPurchaseOrder();
