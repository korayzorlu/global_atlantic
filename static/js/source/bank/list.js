...
function setBankDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/source/bank_add/";
...
/**/let deleteDataUrl = "/source/bank_delete/";
    let serverSide = true;
/**/let apiSource = '/source/api/banks?company=' + sourceCompanyId + '&format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "bankName", className:"double-clickable text-start ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/source/bank_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "company", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "ibanNo", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "swiftNo", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "balance", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
  ...
  /**/htmx.ajax('GET', '/source/bank_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/source/bank_update/' + data["id"] + '/');
    });
