...
export let _componentDatatable = function (manufacturers_api) {
  if (!$().DataTable) {
...
function addEventListeners() {
  ...
  buttonInfo.addEventListener("click", getManufacturerInfo);
}

async function documentReady() {
  document.addEventListener("DOMContentLoaded", function () {
    _componentDatatable(manufacturers_api);
  });
}
...
documentReady()
  .then(() => {
    ...
    // ui.addButtonsForDataTable(manufacturer_add_url);
  })
  .then(() => {
    addEventListeners();
  })
...
function getManufacturerInfo(e) {
  e.preventDefault();
  ...
  if (rows.length === 0) {
    ...
    window.location = `/sales/manufacturer/${rows[0].children[0].innerHTML.trim()}`;
  } else {
    sweetToastType(
