...
export let _componentDatatable = function (related_sets_api) {
  if (!$().DataTable) {
...
function addEventListeners() {
  ...
  buttonInfo.addEventListener("click", getPartInfo);
}

async function documentReady() {
  document.addEventListener("DOMContentLoaded", function () {
    _componentDatatable(related_sets_api);
  });
}
...
documentReady()
  .then(() => {
    ...
    // ui.addButtonsForDataTable(related_set_add_url);
  })
  .then(() => {
    addEventListeners();
  })
...
function getPartInfo(e) {
  ...
  if (rows.length === 0) {
    ...
    window.location = `/sales/related_set/${rows[0].children[0].innerHTML.trim()}`;
  } else {
    sweetToastType(
