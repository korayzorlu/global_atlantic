...
export let _componentDatatable = function (parts_api) {
  if (!$().DataTable) {
  ...
  let table = $(".dataTable").DataTable({
    ...
    columns: [
      ...
      { data: "dimensions", defaultContent: "-" },
...
documentReady()
  .then(() => {
    ...
    // ui.addButtonsForDataTable(part_add_url);
  })
  .then(() => {
    addEventListeners();
  })
...
function getPartInfo(e) {
  ...
  if (rows.length === 0) {
    ...
    window.location = `/sales/part/${rows[0].children[0].innerHTML.trim()}`;
  } else {
    sweetToastType(
