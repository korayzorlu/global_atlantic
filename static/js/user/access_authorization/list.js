...
function setAccessAuthorizationDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/user/access_authorization_add/";
...
/**/let deleteDataUrl = "/user/access_authorization_delete/";
    let serverSide = true;
/**/let apiSource = '/user/api/access_authorizations?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/user/access_authorization_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
  ...
  /**/htmx.ajax('GET', '/user/access_authorization_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/user/access_authorization_update/' + data["id"] + '/');
    });
