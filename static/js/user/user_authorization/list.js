...
function setUserAuthorizationDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/user/user_authorization_add/";
...
/**/let deleteDataUrl = "/user/user_authorization_delete/";
    let serverSide = true;
/**/let apiSource = '/user/api/user_authorizations?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/user/user_authorization_update/' + row.pk + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
    ...
    table.DataTable({
      ...
      drawCallback: function() {
        ...
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td>td></td><td></td><td></td><</tr>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
  ...
  /**/htmx.ajax('GET', '/user/user_authorization_update/' + data["pk"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/user/user_authorization_update/' + data["pk"] + '/');
    });
