...
function setSaleDocumentsDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/library/sale_documents_add/";
...
/**/let deleteDataUrl = "/library/sale_documents_delete/";
    let serverSide = false;
/**/let apiSource = '/media/documents.json';
/**/let columns = [
                    {"data" : "", "width" : "3"},
                    {"data" : "name", "width":"10", render: function (data, type, row, meta)
                        {return '<a hx-get="/library/sale_documents_pdf/' + row.level + '_' + row.name + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "", "width":"10", render: function (data, type, row, meta){
                                    return '<i class="fa-solid fa-file-pdf fa-2xl"></i>'
                    }
                    },
                    {"data" : "level"}
    ];
