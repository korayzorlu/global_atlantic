...
function setOfferDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/service/offer_add/";
...
/**/let deleteDataUrl = "/service/offer_delete/";
    let serverSide = true;
/**/let apiSource = '/service/api/offers?confirmed=false&finished=false&format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "offerNo", className:"double-clickable ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/service/offer_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "offerDate", className:"double-clickable ps-1 pe-1"},
                    {"data" : "customer", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "person", className:"double-clickable ps-1 pe-1"},
                    {"data" : "vessel", className:"double-clickable ps-1 pe-1"},
                    {"data" : "equipmentMaker", className:"double-clickable ps-1 pe-1", render: function (data, type, row, meta){
                        if(data){
                          return data + " || " + row.equipmentType;
                        }else{
                          return "";
                        };
                    }
                    },
                    {"data" : "equipmentType", className:"double-clickable ps-1 pe-1","visible":false},
                    {"data" : "status", className:"double-clickable shadow-0 ps-1 pe-1",visible:false, render: function (data, type, row, meta){
                      if(data == "Offer"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="primary" style="padding:2px 5px;margin:0;"><i class="fas fa-file-invoice me-3"></i>Offer</div>';
                      }else if(data == "Active"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="warning" style="padding:2px 5px;margin:0;"><i class="fas fa-hourglass-half me-3"></i>Active</div>';
                      }else if(data == "Finished"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-check-circle me-3"></i>Finished</div>';
                      };
                    }
                    }
    ];
  ...
  /**/htmx.ajax('GET', '/service/offer_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/service/offer_update/' + data["id"] + '/');
    });
    ...
    if(deleteDataButton){
        $(deleteDataButtonId).click(function (){
            ...
            htmx.ajax("GET", deleteDataUrl + idList, "#addUpdateDataDialog-" + ef);
        });
    };
