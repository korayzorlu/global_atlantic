...
function setNavTabSubDetailActiveProjectAdd(){
    ...
    }else if(elementTag == "activeProjectAdd"){
        var efSub = "activeProject";
    };
...
function formSubmitMessageActiveProjectAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
        ...
        success: function (response, status, xhr) {
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    if($("#navTag-activeProject").length > 0){
                        $("#navTagLink-acceptance").removeClass("active");
                        $("#tabPane-acceptance").removeClass("active show");
                        $("#navTagLink-activeProject").addClass("active");
                        $("#tabPane-activeProject").addClass("active show");

                        fetch('/service/api/active_projects?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                        .then((response) => {
                            return response.json();
                        })
                        .then((data) => {
                            htmx.ajax("GET", "/service/active_project_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
                            window.history.pushState({}, '', "/service/active_project_update/" + data["data"][0]["id"] + "/");
                        });
                    }else{
                        htmx.ajax("GET", "/service/active_project_data", {target:"#tabCont", swap:"afterbegin"});
...
$(document).ready(function () {
    //setNavTabActiveProject();
    setNavTabSubDetailActiveProjectAdd();
    //formSubmitMessageActiveProjectAdd();
});