...
function setActiveProjectDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/service/active_project_add/";
...
/**/let deleteDataUrl = "/service/active_project_delete/";
    let serverSide = true;
/**/let apiSource = '/service/api/offers?confirmed=true&finished=false&format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "offerNo", className:"double-clickable" , render: function (data, type, row, meta)
                        {return '<a hx-get="/service/active_project_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
  ...
  /**/htmx.ajax('GET', '/service/active_project_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/service/active_project_update/' + data["id"] + '/');
    });
...
function setNavTabActiveProject(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
...
/**/setActiveProjectDataTable();
    setNavTabActiveProject();
    setNavTabSubActiveProject();
    setHTMXActiveProject();
