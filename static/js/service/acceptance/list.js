...
function setAcceptanceDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/service/acceptance_add/";
...
/**/let deleteDataUrl = "/service/acceptance_delete/";
    let serverSide = true;
/**/let apiSource = '/service/api/acceptances?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "acceptanceNo", className:"double-clickable ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/service/acceptance_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "acceptanceDate", className:"double-clickable ps-1 pe-1"},
                    {"data" : "customer", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "status", className:"double-clickable shadow-0 ps-1 pe-1", render: function (data, type, row, meta){
                      if(data == "Accepted"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="primary" style="padding:2px 5px;margin:0;"><i class="fas fa-file-invoice me-3"></i>Accepted</div>';
                      }else if(data == "Cancelled"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="danger" style="padding:2px 5px;margin:0;"><i class="fas fa-circle-xmark me-3"></i>Cancelled</div>';
                      }else if(data == "Active"){
                        ...
                        return '<div class="alert text-start" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-circle-check me-3"></i>Finished</div>';
                      };
                    }
                    }
    ];
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        if(offerStatus == "Accepted"){
          $('td:eq(5)', row).addClass("tableAlert-primary");
        }else if(offerStatus == "Cancelled"){
          $('td:eq(5)', row).addClass("tableAlert-danger");
        }else if(offerStatus == "Active"){
            $('td:eq(5)', row).addClass("tableAlert-warning");
        }else if(offerStatus == "Finished"){
            $('td:eq(5)', row).addClass("tableAlert-success");
        };
  ...
  /**/htmx.ajax('GET', '/service/acceptance_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/service/acceptance_update/' + data["id"] + '/');
    });
