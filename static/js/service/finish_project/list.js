...
function setFinishProjectDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/service/finish_project_add/";
...
/**/let deleteDataUrl = "/service/active_project_delete/";
    let serverSide = true;
/**/let apiSource = '/service/api/offers?confirmed=true&finished=true&format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "offerNo", className:"double-clickable" , render: function (data, type, row, meta)
                        {return '<a hx-get="/service/finish_project_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "offerDate", className:"double-clickable"},
                    {"data" : "customer", className:"double-clickable"},
                    {"data" : "person", className:"double-clickable"},
                    ...
                    {"data" : "invoiced", className:"double-clickable shadow-0 ps-1 pe-1",render: function (data, type, row, meta){
                      if(data == true){
                        return '<div class="alert text-start" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-file-invoice me-3"></i>Invoiced</div>';
                      }else if(data == false){
                        return '<div class="alert text-start" role="alert" data-mdb-color="primary" style="padding:2px 5px;margin:0;"><i class="fas fa-circle-check me-3"></i>Finished</div>';
                      };
                    }
                    }
    ];
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        var offerInvoiced = data.invoiced;
        if(offerInvoiced == true){
          $('td:eq(8)', row).css('background-color', 'rgb(214, 240, 224)');
        }else{
  ...
  /**/htmx.ajax('GET', '/service/finish_project_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/service/finish_project_update/' + data["id"] + '/');
    });
...
function setNavTabFinishProject(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
...
function setNavTabSubFinishProject(){
...
/**/setFinishProjectDataTable();
    setNavTabFinishProject();
    setNavTabSubFinishProject();
    setHTMXFinishProject();
