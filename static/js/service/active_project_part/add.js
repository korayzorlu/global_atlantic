function setActiveProjectPartAddDatatable(){
    let es = elementTagSub + "-" + elementTagId + "-partAdd";
...
function formSubmitMessageRequestPartAdd(){
    ...
    $("#form-" + ee +  "-" + ei + "-add-2").submit(function (event) {
      ...
      tableActiveProjectParts = $("#table-" + eSub + "-" + elementTagId + "-partAdd");
      
      for(let i = 0; i < tableActiveProjectParts.DataTable().rows({selected:true}).data().length; i++){
        formData.push({name: "activeProjectParts", value: tableActiveProjectParts.DataTable().rows({selected:true}).data()[i]["id"]});
      };
        ...
        success: function (response, status, xhr) {
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    $("#table-" + eSub + "-" + elementTagId + "-part").DataTable().ajax.reload(function(){
                        htmx.process("#table-" + eSub + "-" + elementTagId + "-part");
                    },false);
...
$(document).ready(function () {
    setActiveProjectPartAddDatatable();
