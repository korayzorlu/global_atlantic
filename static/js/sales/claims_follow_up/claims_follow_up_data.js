...
export let _componentDatatable = function (projects_api_url) {
  ...
  let table = $(".dataTable").DataTable({
    ...
    select: true,
    columns: [
      { data: "pk", defaultContent: "-" },
      { data: "delivery.project.id", defaultContent: "-" },
      { data: "delivery.project.no", defaultContent: "-" },
      { data: "request.customer.name", defaultContent: "-" },
      { data: "delivery.project.project_date", defaultContent: "-" },
      { data: "claim_status", defaultContent: "-" },
...
function getProjectInfo(e) {
  ...
  let rowData = $(".dataTable").DataTable().row(currentRow).data();
  if (rows.length === 0) {
    ...
    window.location = claim_detail_url.replace(0, rowData.pk);
  } else {
    sweetToastType(
  ...
  if (rows.length === 0) {
    ...
    sweetCombineDynamic(
      ...
      () => {
        let rowData = $(".dataTable").DataTable().row(currentRow).data();
        request
          .delete_r(claims_follow_up_update_api_url.replace(0, rowData.pk))
          .then((response) => {
            if (response.ok) {
