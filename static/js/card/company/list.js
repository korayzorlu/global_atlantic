    ...
    let buttons = [
        ...
        // },
        {
          ...
          action: function ( e, dt, node, config ) {
            ...
            htmx.ajax("GET", "/card/company_filter_excel/", {target : "#addUpdateDataDialogXl"});
          }
        },
        // {
        //   extend: "pdfHtml5",
...
/**/let deleteDataUrl = "/card/company_delete/";
    let serverSide = true;
/**/let apiSource = '/card/api/companies?format=datatables';
/**/let columns = [ 
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/card/company_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "hesapKodu", className:"double-clickable ps-1 pe-1"},
                    {"data" : "role", "width" : "6%", className:"double-clickable ps-1 pe-1", render: function (data, type, row, meta){
                    ...
                    {"data" : "country", className:"double-clickable ps-1 pe-1"},
                    {"data" : "city", className:"double-clickable ps-1 pe-1"},
                    {"data" : "phone1", "width" : "10%", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "email", className:"double-clickable ps-1 pe-1"},
                    {"data" : "companyNo", className:"double-clickable ps-1 pe-1"},
                    {"data" : "vatOffice", className:"double-clickable ps-1 pe-1"},
    ...
    /**/htmx.ajax('GET', '/card/company_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/card/company_update/' + data["id"] + '/');
    });
...
function setNavTabCompany(){
  ...
  removeNav.click(function(){
      ...
      if(mainTabState.some(item => item.app === "card" && item.model === "company")){
        let tabIndex = mainTabState.findIndex(item => item.app === "card" && item.model === "company");
        if (tabIndex > -1) {
...
/**/setCompanyDataTable();
    ...
    // if(! mainTabState.some(item => item.app === "card" && item.model === "company")){
    //   mainTabState.push({app : "card", model : "company", url : "/card/card_data/"});
    //   localStorage.setItem('mainTabState', JSON.stringify(mainTabState));
    // };
    /******sekme stroage kontrolü-end******/
