...
function setVesselDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/card/vessel_add/";
...
/**/let deleteDataUrl = "/card/vessel_delete/";
    let serverSide = true;
/**/let apiSource = '/card/api/vessels?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta){
                        ...
                        return '<a hx-get="/card/vessel_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "company", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "owner", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "imo", className:"double-clickable ps-1 pe-1"},
                    {"data" : "mmsi", className:"double-clickable ps-1 pe-1"}
    ];
    ...
    /**/htmx.ajax('GET', '/card/vessel_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/card/vessel_update/' + data["id"] + '/');
    });
