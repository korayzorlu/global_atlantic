...
function setPaymentDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/account/payment_add/";
...
/**/let deleteDataUrl = "/account/payment_delete/";
    let serverSide = true;
/**/let apiSource = '/account/api/payments?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "customer", className:"double-clickable text-start ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/account/payment_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "type", className:"double-clickable ps-1 pe-1"},
                    {"data" : "bank", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "description", className:"double-clickable ps-1 pe-1"},
                    {"data" : "paymentNo", className:"double-clickable ps-1 pe-1"},
                    {"data" : "invoiceAmount", className:"double-clickable text-end ps-1 pe-1" , render: function (data, type, row, meta){
                    ...
                    {"data" : "", className:"double-clickable text-end ps-1 pe-1" , render: function (data, type, row, meta){
                      return '<span class="">' + (row.amount - row.invoiceAmount).toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    ...
                    {"data" : "", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="">' + (row.amount * row.forexBuying).toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    {"data" : "invoices", className:"double-clickable ps-1 pe-1","width":"5%", render: function (data, type, row, meta){
                      invoicesList = []
                      for(let i = 0; i < data.invoices.length; i++){
                        invoicesList.push(data.invoices[i] + "\n");
                      };
                      return invoicesList
                    }
                    },
                    {"data" : "", className:"double-clickable ps-1 pe-1"},
                    {"data" : "forexBuying", className:"double-clickable ps-1 pe-1","visible":false}
    ];
  ...
  /**/htmx.ajax('GET', '/account/payment_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/account/payment_update/' + data["id"] + '/');
    });
