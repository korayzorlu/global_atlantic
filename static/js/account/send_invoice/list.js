...
function setSendInvoiceDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/account/send_invoice_add_manual/";
    ...
    let buttons = [
        ...
        },
        {
          ...
          action: function ( e, dt, node, config ) {
            ...
            htmx.ajax("GET", "/account/send_invoice_filter_excel/", {target : "#addUpdateDataDialogXl"});
          }
        },
        {
...
/**/let deleteDataUrl = "/account/send_invoice_delete/";
    let serverSide = true;
/**/let apiSource = '/account/api/send_invoices?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "", className:"double-clickable shadow-0 ps-1 pe-1", render: function (data, type, row, meta){
                      if(row.group == "order"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="primary" style="padding:2px 5px;margin:0;">Order</div>';
                      }else if(row.group == "service"){
                        return '<div class="alert text-start" role="alert" data-mdb-color="danger" style="padding:2px 5px;margin:0;">Service</div>';
                      };
                    ...
                    //{"data" : "sendInvoiceNoSys", className:"double-clickable ps-1 pe-1"},
                    ...
                    {"data" : "sendInvoiceNo", className:"double-clickable ps-1 pe-1"},
                    ...
                    {"data" : "payed", className:"double-clickable shadow-0 ps-1 pe-1", render: function (data, type, row, meta){
                      if(data){
                        return '<div class="alert" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-circle-check"></i></div>';
                      }else{
                        return '<div class="alert" role="alert" data-mdb-color="warning" style="padding:2px 5px;margin:0;"><i class="fas fa-hourglass-half"></i></div>';
                      };
                    }
                    },
                    {"data" : "ready", className:"double-clickable shadow-0 ps-1 pe-1", render: function (data, type, row, meta){
                      if(data){
                        return '<div class="alert" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-circle-check"></i></div>';
                      }else{
                        return '<div class="alert" role="alert" data-mdb-color="warning" style="padding:2px 5px;margin:0;"><i class="fas fa-hourglass-half"></i></div>';
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        var sendInvoiceGroup = data.group;
        if(sendInvoiceGroup == "order"){
          $('td:eq(2)', row).addClass("tableAlert-primary");
        }else if(sendInvoiceGroup == "service"){
          $('td:eq(2)', row).addClass("tableAlert-danger");
        };

        var sendInvoicePaid = data.payed;
        if(sendInvoicePaid){
          $('td:eq(17)', row).addClass("tableAlert-success");
        }else{
          $('td:eq(17)', row).addClass("tableAlert-warning");
        };

        var sendInvoiceReady = data.ready;
        if(sendInvoiceReady){
          $('td:eq(18)', row).addClass("tableAlert-success");
        }else{
          $('td:eq(18)', row).addClass("tableAlert-warning");
        };
      ...
      drawCallback: function() {
        ...
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
