...
function formSubmitMessageSendInvoiceAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
            ...
            success: function (response, status, xhr) {
                ...
                if (xhr.status === 204) {
                    setTimeout(function() {
                        ...
                        if($("#navTag-sendInvoice").length > 0){
                            $("#navTagLink-orderInProcessA").removeClass("active");
                            $("#tabPane-orderInProcessA").removeClass("active show");
                            $("#navTagLink-sendInvoice").addClass("active");
                            $("#tabPane-sendInvoice").addClass("active show");

                            fetch('/account/api/send_invoices?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                            .then((response) => {
                                return response.json();
                            })
                            .then((data) => {
                                htmx.ajax("GET", "/account/send_invoice_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
                                window.history.pushState({}, '', "/account/send_invoice_update/" + data["data"][0]["id"] + "/");
                            });
                        }else{
                            htmx.ajax("GET", "/account/send_invoice_data", {target:"#tabCont", swap:"afterbegin"});
                            setNavTabSendInvoice();
                            fetch('/account/api/send_invoices?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                            .then((response) => {
                                return response.json();
                            })
...
$(document).ready(function () {
    formSubmitMessageSendInvoiceAdd();
});