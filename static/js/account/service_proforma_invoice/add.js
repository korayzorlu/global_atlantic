function setNavTabServiceProformaInvoice(){
    let e = elementTag;
    let ef = elementTag;
    let u = pageUrl;
...
function formSubmitMessageServiceProformaInvoiceAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
                ...
                if (xhr.status === 204) {
                    setTimeout(function() {
                        ...
                        if($("#navTag-proformaInvoice").length > 0){
                            ...
                            setNavTabServiceProformaInvoice();
                            fetch('/account/api/proforma_invoices?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                            .then((response) => {
                                return response.json();
                            })
...
$(document).ready(function () {
    formSubmitMessageServiceProformaInvoiceAdd();
});