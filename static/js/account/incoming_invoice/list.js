...
function setIncomingInvoiceDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/account/incoming_invoice_add_manual/";
...
/**/let deleteDataUrl = "/account/incoming_invoice_delete/";
    let serverSide = true;
/**/let apiSource = '/account/api/incoming_invoices?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "group", className:"double-clickable shadow-0 ps-1 pe-1", render: function (data, type, row, meta){
                      if(data == "order"){
                        ...
                        return '<div class="alert text-start" role="alert" data-mdb-color="dark" style="padding:2px 5px;margin:0;">Purchasing</div>';
                      };
                    ...
                    {"data" : "seller", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/account/incoming_invoice_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    ...
                    {"data" : "incomingInvoiceNo", className:"double-clickable ps-1 pe-1"},
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        }else if(sendInvoiceGroup == "purchasing"){
          $('td:eq(2)', row).addClass("tableAlert-dark");
        };
    ...
    /**/htmx.ajax('GET', '/account/incoming_invoice_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/account/incoming_invoice_update/' + data["id"] + '/');
    });
