function setCommericalInvoicePartDetailDatatable(){
...
/**/let addDataHxGet = "/sale/inquiry_part_add_in_detail/";
    ...
    let editor = new $.fn.dataTable.Editor({
        ajax: "/account/api/commerical_invoice_items/editor/",
        table: tableId,
        idSrc: "id",
        fields: [{
                label: "trDescription",
...
/**/let deleteDataUrl = "/account/commerical_invoice_item_delete/";
    let serverSide = false;
/**/let apiSource = '/account/api/commerical_invoice_items?invoice=' + commericalInvoiceId + '&format=datatables';
/**/let columns = [
        {
            orderable: false,
        ...
        {"data" : "unitPrice", className:"text-end ps-1 pe-1", "width": "5%", render: function (data, type, row, meta)
                            {return row.currency + " " + (data).toFixed(2)}
        },
        {"data" : "totalPrice", className:"text-end ps-1 pe-1", "width": "5%", render: function (data, type, row, meta)
                            {return row.currency + " " + (data).toFixed(2)}
        },
...
function setCommericalInvoiceExpenseDetailDatatable(){
    let es = elementTagSub + "-" + elementTagId + "-expense";
    let id = commericalInvoiceId
    ...
    let tableId = '#table-' + es;
    let table = $('#table-' + es);
/**/let addDataHxGet = "/account/commerical_invoice_expense_add_in_detail/i_" + id + "/";
    let addDataHxTarget = "#addFormBlock-inform-d-justDetail-" + es;
    ...
    //Datatable Editor için editor'ü tanımlar.
    let editor = new $.fn.dataTable.Editor({
        ajax: "/account/api/commerical_invoice_expenses/editor/",
        table: tableId,
        idSrc: "id",
        fields: [{
            label: "quantity",
    ...
    let deleteDataButton = $('.deleteData');
    let deleteDataButtonId = ".delete-" + es;
/**/let deleteDataUrl = "/account/commerical_invoice_expense_delete/";
    let serverSide = true;
/**/let apiSource = '/account/api/commerical_invoice_expenses?invoice=' + commericalInvoiceId + '&format=datatables';
/**/let columns = [
        {
            orderable: false,
            searchable: false,
        ...
        {"data" : "unitPrice", className:"editable",render: function (data, type, row, meta)
            {return row.invoice.currency + " " + (data).toFixed(2)}
        },
        {"data" : "totalPrice", className:"editable", render: function (data, type, row, meta)
            {return row.invoice.currency + " " + (data).toFixed(2)}
        },
        {"data" : "invoice.currency", "visible" : false},
    ];
...
function formSubmitMessageCommericaldInvoice(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
...
function setNavTabSubCommericalInvoice(){
...
$(document).ready(function () {
    setCommericalInvoicePartDetailDatatable();
    setCommericalInvoiceExpenseDetailDatatable();
    setHTMX();
    setNavTabSubCommericalInvoice();
    formSubmitMessageCommericaldInvoice();
});