...
function setSendInvoiceDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/account/proforma_invoice_add_manual/";
...
/**/let deleteDataUrl = "/account/proforma_invoice_delete/";
    let serverSide = true;
/**/let apiSource = '/account/api/proforma_invoices?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "customer", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta){
                      if(row.group == "order"){
                        return '<a hx-get="/account/proforma_invoice_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                      }else if(row.group == "service"){
                        return '<a hx-get="/account/proforma_invoice_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                      };
                    ...
                    {"data" : "proformaInvoiceDate", className:"double-clickable ps-1 pe-1"},
                    {"data" : "proformaInvoiceNo", className:"double-clickable ps-1 pe-1"},
    ...
    table.DataTable({
      ...
      drawCallback: function() {
        ...
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
    ...
    table.on('dblclick', '.double-clickable', function () {
        ...
        /**/htmx.ajax('GET', '/account/proforma_invoice_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/account/proforma_invoice_update/' + data["id"] + '/');
