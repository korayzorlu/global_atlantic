...
export let _componentDatatable = function (teams_api) {
  if (!$().DataTable) {
...
function addEventListeners() {
  ...
  buttonInfo.addEventListener("click", getTeamInfo);
}

async function documentReady() {
  document.addEventListener("DOMContentLoaded", function () {
    _componentDatatable(teams_api);
  });
}
...
documentReady()
  .then(() => {
    ...
    // ui.addButtonsForDataTable(team_add_url);
  })
  .then(() => {
    addEventListeners();
  })
...
// Required function to go to team page
function getTeamInfo(e) {
  ...
  if (rows.length === 0) {
    sweetToastType("Select a team.", "warning");
  } else if (rows.length === 1) {
    window.localStorage.setItem(
      "pageStatus",
      document.getElementById("pageMainRoute").innerHTML
    );
    window.location = `/hr/team/${rows[0].children[0].innerHTML.trim()}`;
  } else {
    sweetToastType(
      "You can choose at most one team to go to the information page.",
      "warning"
    );
  }
}
