function formSubmitMessageTeamAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
    ...
    $("#form-" + ee +  "-" + ei).submit(function (event) {
        ...
        setTimeout(function() {
            ...
            fetch('/user/api/teams?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                .then((response) => {
                return response.json();
                })
                .then((data) => {
                console.log(data["data"][0]["id"]);
                htmx.ajax("GET", "/hr/team_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
                window.history.pushState({}, '', "/hr/team_update/" + data["data"][0]["id"] + "/");
