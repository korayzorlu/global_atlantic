...
function setDispatchDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/warehouse/dispatch_add/";
...
/**/let deleteDataUrl = "/warehouse/dispatch_delete/";
    let serverSide = true;
/**/let apiSource = '/warehouse/api/dispatches?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "dispatchNo", className:"double-clickable ps-1 pe-1"}
    ];
  ...
  /**/htmx.ajax('GET', '/warehouse/dispatch_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/warehouse/dispatch_update/' + data["id"] + '/');
    });
...
/**/setDispatchDataTable();
    setNavTabDispatch();
    setNavTabSubDispatch();
    setHTMXDispatch();
