function setCariHesaplarDataTable(){/**/
...
/**/let deleteDataUrl = "/mikro/cari_hesaplar_delete/";
    let serverSide = false;
/**/let apiSource = '/mikro/api/cari_hesaplar?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "unvan1", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta){
                      let cariKod = row.kod.replace(/\./g, "_");
                      return '<a hx-get="/mikro/cari_hesaplar_update/' + cariKod + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    }
                    },
                    {"data" : "unvan2", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "vDaireAdi", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "vDaireNo", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "ulke", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "sehir", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "adres", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "doviz", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "doviz1", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "doviz2", className:"double-clickable text-start ps-1 pe-1"}
    ];
    ...
    table.DataTable({
      ...
      drawCallback: function() {
        ...
        if(api.rows().data().length == 0){
            $(tableId + ' tbody').prepend($("<tr ><td class='text-center fw-bold text-white' colspan='11' style='background-color:#9d2235;'><i class='fa-solid fa-link-slash'></i> Failed to connect to Mikro</td></tr>"));
        };
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
    ...
    table.on('dblclick', '.double-clickable', function () {
        ...
        /**/htmx.ajax('GET', '/mikro/cari_hesaplar_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/mikro/cari_hesaplar_update/' + data["id"] + '/');
