function setCariHesapAdresleriDataTable(){/**/
...
/**/let deleteDataUrl = "/mikro/cari_hesap_adresleri_delete/";
    let serverSide = false;
/**/let apiSource = '/mikro/api/cari_hesap_adresleri?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "mahalle", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "cadde", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "sokak", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "postaKodu", className:"double-clickable ps-1 pe-1"},
                    {"data" : "ilce", className:"double-clickable ps-1 pe-1"},
                    {"data" : "il", className:"double-clickable ps-1 pe-1"},
                    {"data" : "ulke", className:"double-clickable ps-1 pe-1"}
    ];
    ...
    table.DataTable({
      ...
      drawCallback: function() {
        ...
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td></td>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
    ...
    table.on('dblclick', '.double-clickable', function () {
        ...
        /**/htmx.ajax('GET', '/mikro/cari_hesap_adresleri_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/mikro/cari_hesap_adresleri_update/' + data["id"] + '/');
...
$(document).ready(function () {
    /**/setCariHesapAdresleriDataTable();
        setNavTab();
        setNavTabSub();
        setHTMX();
