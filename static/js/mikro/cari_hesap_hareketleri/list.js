function setCariHesapHareketleriDataTable(){/**/
...
/**/let deleteDataUrl = "/mikro/cari_hesap_hareketleri_delete/";
    let serverSide = true;
/**/let apiSource = '/mikro/api/cari_hesap_hareketleri?format=datatables';
/**/let columns = [
                      {
                        orderable: false,
                        searchable: false,
                    ...
                    // }
                    // },
                    {"data" : "tarih", className:"double-clickable ps-1 pe-1"},
                    {"data" : "kod", className:"double-clickable ps-1 pe-1"},
                    {"data" : "cari", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "evrakTip", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "belgeNo", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "proje", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "araToplam", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="">' + data.toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    {"data" : "iskonto", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="discountPriceText">' + data.toLocaleString('tr-TR', { minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    {"data" : "vergi", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="">' + data.toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    {"data" : "meblag", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="totalPriceText">' + data.toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    {"data" : "doviz", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "kur", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="">' + data.toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    },
                    {"data" : "", className:"double-clickable text-end ps-1 pe-1", render: function (data, type, row, meta){
                      return '<span class="">' + (row.meblag * row.kur).toLocaleString('tr-TR', { currency: row.currency, minimumFractionDigits: 2 }) + '</span>';
                    }
                    }
    ];
    ...
    table.DataTable({
      ...
      drawCallback: function() {
        ...
        if(api.rows().data().length == 0){
            $(tableId + ' tbody').prepend($("<tr ><td class='text-center fw-bold text-white' colspan='12' style='background-color:#9d2235;'><i class='fa-solid fa-link-slash'></i> Failed to connect to Mikro</td></tr>"));
        };
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td>"));
        }
      },
      "ajax" : apiSource,
      "columns" : columns
...
$(document).ready(function () {
    /**/setCariHesapHareketleriDataTable();
        setNavTab();
        setNavTabSub();
        setHTMX();
