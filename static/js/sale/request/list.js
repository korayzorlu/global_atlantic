...
function setRequestDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/sale/request_add/";
    ...
    let buttons = [
        ...
        },
        {
          // text: '<i class="fa-solid fa-plus" data-mdb-toggle="tooltip" data-mdb-placement="right" title="Add a new"></i>',
          tag: "img",
          attr: {src:"/static/images/icons/datatable/filter.svg",id: ef + "-filter"},
          className: "tableTopButtons inTableButtons",
          action: function ( e, dt, node, config ) {
            $("#" + ef + "-filterBlock").toggle("open");
...
/**/let deleteDataUrl = "/sale/request_delete/";
    let serverSide = true;
/**/let apiSource = '/sale/api/requests?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "pk", className:"double-clickable", "visible":false},
                    {"data" : "requestNo", className:"double-clickable ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/sale/request_update/' + row.pk + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "requestDate", className:"double-clickable ps-1 pe-1"},
                    ...
                    {"data" : "note", className:"double-clickable ps-1 pe-1"},
    ...
    table.DataTable({
      ...
      drawCallback: function() {
        ...
        for (var i = 0; i < api.page.len() - (rowCount === 0? 1 : rowCount); i++) {
          $(tableId + ' tbody').append($("<tr ><td>&nbsp;</td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>"));
        }

        // $(".tableBox-" + ef + " .dataTables_scroll").before(
        //   `
        //     <div id="` + ef + `-filterBlock" class="">test</div>
        //   `
        // );
  ...
  /**/htmx.ajax('GET', '/sale/request_update/' + data["pk"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/sale/request_update/' + data["pk"] + '/');
    });
