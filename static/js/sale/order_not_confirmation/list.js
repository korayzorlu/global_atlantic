...
function setOrderNotConfirmationDataTable(){/**/
...
/**/let deleteDataUrl = "/sale/order_not_confirmation_delete/";
    let serverSide = true;
/**/let apiSource = '/sale/api/order_not_confirmations?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "project.projectNo", className:"double-clickable"},
                    {"data" : "orderNotConfirmationNo", className:"double-clickable" , render: function (data, type, row, meta)
                        {return '<a hx-get="/sale/order_not_confirmation_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "quotation.quotationNo", className:"double-clickable"},
                    {"data" : "quotation.quotationDate", className:"double-clickable"},
                    {"data" : "quotation.inquiry.theRequest.customer_name", className:"double-clickable"},
                    {"data" : "quotation.inquiry.theRequest.vessel.name", className:"double-clickable"},
                    {"data" : "project.user.first_name", className:"double-clickable" , render: function (data, type, row, meta){
                      return data + ' ' + row.project.user.last_name;
                    }
                    },
                    {"data" : "project.user.last_name", className:"double-clickable"},
                    {"data" : "quotation.id", className:"double-clickable"}
    ];
    ...
    /**/htmx.ajax('GET', '/sale/order_not_confirmation_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
...
/**/setOrderNotConfirmationDataTable();
    setNavTab();
    setNavTabSub();
    setHTMX();
