...
function setRequestPartInquiryAddDataTable(){/**/
    ...
    $("#tabPane-" + es).busyLoad("show", {
      ...
      minSize: "150px",
      text: "Loading ...",
      background: theme == "light" ? "#fff" : theme == "dark" ? "#383838" : "#fff",
      color: "#455359",
      textColor: "#455359"
    });
...
/**/let apiSource = '/sale/api/request_parts?theRequest=' + elementTagId + '&format=datatables';
/**/let columns = [
                    { 
                      orderable: false,
...
function setCompanyInquiryAddDataTable(){/**/
    // /**/ olan alanlar ilgili modele göre zorunlu değişecek alanlar
...
function setNavTabSubInquiryAdd(){
...
function setNavTabSubDetailInquiryAdd(){
...
function formSubmitMessageInquiryAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl;
    let sk = sessionKey;
    ...
    $("#form-" + ee +  "-" + ei + "-add-2").submit(function (event) {
      ...
      tableParts = $("#table-" + elementTagSub + "-" + elementTagId + "-part");
      ...
      for(let i = 0; i < tableParts.DataTable().rows({selected:true}).data().length; i++){
        formData.push({name: "parts", value: tableParts.DataTable().rows({selected:true}).data()[i]["id"]});
      };
...
/**/setRequestPartInquiryAddDataTable();
/**/setCompanyInquiryAddDataTable();
    setNavTabSubDetailInquiryAdd();
    //formSubmitMessageInquiryAdd();
