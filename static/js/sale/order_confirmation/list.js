...
function setOrderConfirmationDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/sale/order_confirmation_add/";
    ...
    let buttons = [
        ...
        {
          tag: "img",
          attr: {src:"/static/images/icons/datatable/xls.svg",href:"/sale/order_confirmation_all_excel/"}, 
          className: "tableTopButtons inTableButtons",
          action: function ( e, dt, node, config ) {
            //window.location.href = "/sale/order_confirmation_all_excel/";
            //htmx.ajax("GET", "/sale/quotation_all_excel/", {target : "#addUpdateDataDialog-inform"});
            htmx.ajax("GET", "/sale/order_confirmation_filter_excel/", {target : "#addUpdateDataDialogXl"});
          }
        },
        // {
        //   extend: "pdfHtml5",
...
/**/let deleteDataUrl = "/sale/order_confirmation_delete/";
    let serverSide = true;
/**/let apiSource = '/sale/api/order_confirmations?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "orderConfirmationNo", className:"double-clickable ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/sale/order_confirmation_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "orderConfirmationDate", className:"double-clickable ps-1 pe-1"},
                    {"data" : "customerRef", className:"double-clickable ps-1 pe-1"},
                    ...
                    {"data" : "quotationId", className:"double-clickable ps-1 pe-1" ,"visible" : false},
                    {"data" : "projectStage", className:"double-clickable text-start shadow-0 ps-1 pe-1", "width" : "9%", render: function (data, type, row, meta){
                      if(data == "request"){
    ...
    table.DataTable({
      ...
      createdRow: function(row, data, index) {
        ...
        if(projectStage == "request" || projectStage == "inquiry" || projectStage == "quotation"){
          $('td:eq(13)', row).addClass("tableAlert-secondary");
        }else if(projectStage == "order_confirmation" || projectStage == "purchase_order"){
          $('td:eq(13)', row).addClass("tableAlert-primary");
        }else if(projectStage == "order_tracking"){
          $('td:eq(13)', row).addClass("tableAlert-warning");
        }else if(projectStage == "invoiced"){
          $('td:eq(13)', row).addClass("tableAlert-success");
        };
      },
    ...
    /**/htmx.ajax('GET', '/sale/order_confirmation_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/sale/order_confirmation_update/' + data["id"] + '/');
    });
