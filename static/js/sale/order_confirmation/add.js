function setNavTabOrderConfirmation(){
    let e = elementTag;
    let ef = elementTag;
    let u = pageUrl;
...
function setNavTabSubDetailOrderConfirmationAdd(){
...
function formSubmitMessageOrderConfirmationAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
        ...
        success: function (response, status, xhr) {
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    if($("#navTag-orderConfirmation").length > 0){
                        $("#navTagLink-quotation").removeClass("active");
                        $("#tabPane-quotation").removeClass("active show");
                        $("#navTagLink-orderConfirmation").addClass("active");
                        $("#tabPane-orderConfirmation").addClass("active show");

                        fetch('/sale/api/order_confirmations?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                        .then((response) => {
                            return response.json();
                        })
                        .then((data) => {
                            htmx.ajax("GET", "/sale/order_confirmation_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
                            window.history.pushState({}, '', "/sale/order_confirmation_update/" + data["data"][0]["id"] + "/");
                        });
                    }else{
                        htmx.ajax("GET", "/sale/order_confirmation_data", {target:"#tabCont", swap:"afterbegin"});
...
$(document).ready(function () {
    //setNavTabOrderConfirmation();
    setNavTabSubDetailOrderConfirmationAdd();
    //formSubmitMessageOrderConfirmationAdd();
});