function setNavTabQuotationRevision(){
    let e = elementTag;
    let ef = elementTag;
    let u = pageUrl;
...
function setNavTabSubDetailQuotationRevision(){
    let eSub = elementTag + "-" + elementTagId + "-revision";
  
    if(elementTag == "quotationRevision"){
      var efSub = "quotation";
    }
...
function formSubmitMessageQuotationRevision(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
    console.log(u);
    $("#form-" + ee +  "-" + ei + "-revision").submit(function (event) {
        //spinner başlatır
        $("body").busyLoad("show", {
            animation: false,
            spinner: "pulsar",
      ...
      $.ajax({
        ...
        success: function (response, status, xhr) {
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    if($("#navTag-quotation").length > 0){
                        $("#navTagLink-quotation").removeClass("active");
                        $("#tabPane-quotation").removeClass("active show");
                        $("#navTagLink-quotation").addClass("active");
                        $("#tabPane-quotation").addClass("active show");

                        $("#navTag-quotation-" + ei).remove();
                        $("#tabPane-quotation-" + ei).remove();

                        fetch('/sale/api/quotations?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                        .then((response) => {
                            return response.json();
                        })
                        .then((data) => {
                            htmx.ajax("GET", "/sale/quotation_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
                            window.history.pushState({}, '', "/sale/quotation_update/" + data["data"][0]["id"] + "/");
                        });
                    }else{
                        htmx.ajax("GET", "/sale/quotation_data", {target:"#tabCont", swap:"afterbegin"});
                        setNavTabQuotationRevision();
                        fetch('/sale/api/quotations?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                        .then((response) => {
                            return response.json();
                        })
                ...
                $("#message-container-" + ee + '-' + ei + "-revision").html('<div id="message-container-inside-' + ee + '-' + ei + "-revision" + '"><i class="fas fa-check-circle me-1"></i>Saved!</div>');
                // Mesajı belirli bir süre sonra gizle
                setTimeout(function() {
                  $("#message-container-inside-" + ee + '-' + ei + "-revision").fadeOut("slow");
        ...
        error: function (xhr, status, error) {
            ...
            $("#message-container-" + ee + '-' + ei + "-revision").html('<div id="message-container-inside-' + ee + '-' + ei + "-revision" + '"><i class="fas fa-xmark-circle me-1"></i>At least one part must be selected!</div>');
        }
      });
...
$(document).ready(function () {
    //setNavTabOrderConfirmation();
    setNavTabSubDetailQuotationRevision();
    formSubmitMessageQuotationRevision();
});