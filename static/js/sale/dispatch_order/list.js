...
function setDispatchOrderDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/sale/dispatch_order_add_manual/";
...
/**/let deleteDataUrl = "/sale/dispatch_order_delete/";
    let serverSide = true;
/**/let apiSource = '/sale/api/dispatch_orders?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "dispatchOrderNo", className:"double-clickable ps-1 pe-1"}
    ];
  ...
  /**/htmx.ajax('GET', '/sale/dispatch_order_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/sale/dispatch_order_update/' + data["id"] + '/');
    });
...
function setNavTabDispatchOrder(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
