function setNavTabPurchaseOrder(){
    let e = elementTag;
    let ef = elementTag;
    let u = pageUrl;
...
function setNavTabSubDetailPurchaseOrderAdd(){
...
function formSubmitMessagePurchaseOrderAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    if($("#navTag-purchaseOrder").length > 0){
                        ...
                        fetch('/sale/api/purchase_orders?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                        .then((response) => {
                            return response.json();
                        })
                        .then((data) => {
                            htmx.ajax("GET", "/sale/purchase_order_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
...
$(document).ready(function () {
    //setNavTabPurchaseOrder();
    setNavTabSubDetailPurchaseOrderAdd();
