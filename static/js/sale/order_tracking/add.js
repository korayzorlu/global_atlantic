function setNavTabOrderTracking(){
    let e = elementTag;
    let ef = elementTag;
    let u = pageUrl;
...
function setNavTabSubDetailOrderTrackingAdd(){
    ...
    }else if(elementTag == "orderTrackingAdd"){
        var efSub = "orderTracking";
    };
...
function formSubmitMessageOrderTrackingAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    if($("#navTag-orderTracking").length > 0){
                        ...
                        fetch('/sale/api/order_trackings?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                        .then((response) => {
                            return response.json();
                        })
                        .then((data) => {
                            htmx.ajax("GET", "/sale/order_tracking_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
...
$(document).ready(function () {
    //setNavTabOrderTracking();
    setNavTabSubDetailOrderTrackingAdd();
    //formSubmitMessageOrderTrackingAdd();
});