...
function setOrderTrackingDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/sale/order_tracking_add/";
...
/**/let deleteDataUrl = "/sale/order_tracking_delete/";
    let serverSide = true;
...
/**/let columns = [
                    ...
                    {"data" : "projectNo", className:"double-clickable ps-1 pe-1" , render: function (data, type, row, meta)
                        {return '<a hx-get="/sale/order_tracking_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    ...
                    {"data" : "collections", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta){
                      let collections = row.collections
                      let collectionList = []
                      for(let i = 0; i < collections.length; i++){
                        collectionList.push(collections[i]["trackingNo"]);
                      };
                      return collectionList;
                    }
                    },
                    {"data" : "projectCreator", className:"double-clickable ps-1 pe-1"},
                    {"data" : "items", "width" : "12%", className:"double-clickable shadow-0 ps-1 pe-1", render: function (data, type, row, meta){
                      if(data){
                        if(data["parts"]["remaining"] > 0){
                          return '<div class="alert" role="alert" data-mdb-color="warning" style="padding:2px 5px;margin:0;"><i class="fas fa-exclamation-triangle me-3"></i>' + data["parts"]["remaining"] + ' remaining parts</div>'
                        }else{
                          return '<div class="alert" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-check-circle me-3"></i>All parts completed</div>'
                        };
                      }else{
                        return "";
                      };
                    ...
                    {"data" : "collected", "width" : "10%", className:"double-clickable shadow-0 ps-1 pe-1" , render: function (data, type, row, meta){
                      if(row.delivered == true){
                        return '<div class="alert" role="alert" data-mdb-color="success" style="padding:2px 5px;margin:0;"><i class="fas fa-truck me-3"></i>Delivered</div>';
                      }else if(row.invoiced == true){
                        return '<div class="alert" role="alert" data-mdb-color="info" style="padding:2px 5px;margin:0;"><i class="fas fa-truck me-3"></i>Delivering</div>';
                      }else if(data == true){
                        return '<div class="alert" role="alert" data-mdb-color="warning" style="padding:2px 5px;margin:0;"><i class="fas fa-dolly me-3"></i>Collected</div>';
                      }else if(data == false){
    ...
    table.DataTable({
      ...
      //   var projectClosed = data.projectClosed;
      //   if(projectClosed == true){
      //     $('td:eq(2)', row).css('background-color', '#1DE9B6');
      //   }else{
      //     $('td:eq(2)', row).css('background-color', '#FF1744');
      //   };
      ...
      createdRow: function(row, data, index) {
        var orderTrackingRemaining = data.items;
        if(orderTrackingRemaining){
          if(orderTrackingRemaining["parts"]["remaining"] > 0){
            $('td:eq(8)', row).addClass("tableAlert-warning");
          }else{
            $('td:eq(8)', row).addClass("tableAlert-success");
          };
        };

        var orderTrackingdelivered = data.delivered;
        var orderTrackingCollected = data.collected;
        var orderTrackingInvoiced = data.invoiced;
        if(orderTrackingdelivered == true){
          $('td:eq(9)', row).addClass("tableAlert-success");
        }else if(orderTrackingInvoiced == true){
          $('td:eq(9)', row).addClass("tableAlert-info");
        }else if(orderTrackingCollected == true){
          $('td:eq(9)', row).addClass("tableAlert-warning");
        }else if(orderTrackingCollected == false){
    ...
    /**/htmx.ajax('GET', '/sale/order_tracking_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/sale/order_tracking_update/' + data["id"] + '/');
    });
...
/**/setOrderTrackingDataTable();
    setNavTab();
    setNavTabSub();
    setHTMXOrderTracking();
