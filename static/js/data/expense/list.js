...
function setExpenseDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/data/expense_add/";
...
/**/let deleteDataUrl = "/data/expense_delete/";
    let serverSide = true;
/**/let apiSource = '/data/api/expenses?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "code", className:"double-clickable", render: function (data, type, row, meta){
                        return '<a hx-get="/data/expense_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },          
    ...
    /**/htmx.ajax('GET', '/data/expense_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
        window.history.pushState({}, '', '/data/expense_update/' + data["id"] + '/');
    });
