...
function setMakerDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/data/maker_add/";
...
/**/let deleteDataUrl = "/data/maker_delete/";
    let serverSide = true;
/**/let apiSource = '/data/api/makers?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable", render: function (data, type, row, meta)
                        {return '<a hx-get="/data/maker_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    },
                    {"data" : "info", className: 'editable double-clickable'}
    ];
  ...
  /**/htmx.ajax('GET', '/data/maker_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/data/maker_update/' + data["id"] + '/');
    });
