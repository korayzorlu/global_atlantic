function setMakerTypeAddDatatable(){
    ...
    let table = $('#table-' + es);
/**/let addDataHxGet = "/data/maker_type_add/";
    ...
    let editor = new $.fn.dataTable.Editor({
        ajax: "/data/api/maker_types_add/editor/",
        table: tableId,
        idSrc: "id",
        fields: [{
            label: "user",
...
/**/let apiSource = '/data/api/maker_types_add?maker__isnull=true&sessionKey=' + sessionKey + '&format=datatables';
/**/let columns = [
        {
            orderable: false,
...
function formSubmitMessageMakerAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl
    ...
    $("#form-" + ee +  "-" + ei).submit(function (event) {
        ...
        setTimeout(function() {
            ...
            fetch('/data/api/makers?ordering=-id&sessionKey=' + sessionKey + '&user=' + user + '&format=datatables', {method : "GET"})
                .then((response) => {
                return response.json();
                })
                .then((data) => {
                console.log(data["data"][0]["id"]);
                htmx.ajax("GET", "/data/maker_update/" + data["data"][0]["id"] + "/", {target:"#tabContSub-"+ee, swap:"afterbegin"});
                window.history.pushState({}, '', "/data/maker_update/" + data["data"][0]["id"] + "/");
...
$(document).ready(function () {
    setMakerTypeAddDatatable();
    setHTMX();
    setNavTabSubMakerAdd();
    formSubmitMessageMakerAdd();
});