function setPartExcelMakerDatatable(){
    let es = elementTagSub + "-maker";
...
/**/let apiSource = '/data/api/part_makers?format=datatables';
/**/let columns = [
        {
            orderable: false,
            searchable: false,
            className: 'select-checkbox p-1',
            targets: 0,
            "width": "1%"
        },
        {"data" : "",className:"p-1","width": "3%"},
        {"data" : "id",className:"p-1","visible":false},
        {"data" : "name", className:"double-clickable text-start p-1"}
    ];
    ...
    table.DataTable().on('select', function(e, dt, type, indexes){
        $("#table-partPartExcel-type").busyLoad("show", {
            animation: false,
            spinner: "pulsar",
            maxSize: "150px",
            minSize: "150px",
        ...
        if (type === 'row') {
            ...
            for(let i = 0; i < table.DataTable().rows({selected:true}).data().length; i++){
                selectedMakers.push(table.DataTable().rows({selected:true}).data()[i]["id"]);
            };
            console.log(selectedMakers);
            
        };

        $("#table-partPartExcel-type").DataTable().ajax.url('/data/api/part_types?maker='+selectedMakers+'&format=datatables').load();

        $("#table-partPartExcel-type").DataTable().ajax.reload()

        $("#table-partPartExcel-type").on( 'draw.dt', function () {
            htmx.process("#table-partPartExcel-type");
            $("#table-partPartExcel-type").busyLoad("hide", {
                animation: "fade"
            });
        });
    });
...
function setPartExcelTypeDatatable(){
    let es = elementTagSub + "-type";
...
/**/let deleteDataUrl = "/sale/request_part_delete/";
    let serverSide = false;
/**/let apiSource = '/data/api/part_types?format=datatables';
/**/let columns = [
        {
            orderable: false,
            searchable: false,
        ...
        {"data" : "",className:"p-1","width": "3%"},
        {"data" : "id",className:"p-1","visible":false},
        {"data" : "type", className:"double-clickable text-start p-1"},
        {"data" : "maker", className:"double-clickable text-start p-1"}
    ];
...
$(document).ready(function () {
    setPartExcelMakerDatatable();
    setPartExcelTypeDatatable();
});