...
function setSourceCompanyDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/administration/company_add/";
...
/**/let deleteDataUrl = "/administration/company_delete/";
    let serverSide = true;
/**/let apiSource = '/administration/api/companies?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "companyNo", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/administration/company_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "formalName", className:"double-clickable text-start ps-1 pe-1"}
    ];
  ...
  /**/htmx.ajax('GET', '/administration/company_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/administration/company_update/' + data["id"] + '/');
    });
...
function setNavTabSourceCompany(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
...
function setNavTabSubSourceCompany(){
...
/**/setSourceCompanyDataTable();
    setNavTabSourceCompany();
    setNavTabSubSourceCompany();
    setHTMXSourceCompany();
    formSubmitMessage();
