function setUserSourceCompanyDetailDatatable(){
    ...
    let table = $('#table-' + es);
/**/let addDataHxGet = "/administration/user_source_company_add/u_" + id + "/";
...
/**/let apiSource = '/administration/api/user_source_companies?userId=' + userId + '&format=datatables';
/**/let columns = [
        {
            orderable: false,
            searchable: false,
        ...
        {"data" : "id"},
        {"data" : "name", "width": "15%", className:"text-start ps-1 pe-1"},
        {"data" : "formalName", className:"text-start ps-1 pe-1", "width": "72%"}
    ];
    ...
    if(deleteDataButton){
        $(deleteDataButtonId).click(function (){
          ...
          //işlemler
          console.log(table.DataTable().row({selected:true}).data());
          ...
          htmx.ajax("POST", deleteDataUrl + "u_" + userId + "_list_" + idList, {target : "#addUpdateDataDialog-inform", headers : "{'X-CSRFToken': '{{ csrf_token }}'}"});
          setTimeout(function(){
              table.DataTable().ajax.reload(function() {
                  htmx.process(tableId);
              }, false);
...
function formSubmitMessageUserList(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl;
...
$(document).ready(function () {
    setUserSourceCompanyDetailDatatable();
    //setHTMX();
    setNavTabSubUserList();
    formSubmitMessageUserList();
