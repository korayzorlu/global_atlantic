...
function setUserListDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/administration/user_add/";
...
/**/let deleteDataUrl = "/administration/user_delete/";
    let serverSide = true;
/**/let apiSource = '/administration/api/users?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/administration/user_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                    ...
                    {"data" : "sourceCompany", className:"double-clickable text-start ps-1 pe-1"}
    ];
  ...
  /**/htmx.ajax('GET', '/administration/user_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/administration/user_update/' + data["id"] + '/');
    });
...
/**/setUserListDataTable();
    setNavTabUserList();
    setNavTabSubUserList();
    setHTMXUserList();
    formSubmitMessage();
