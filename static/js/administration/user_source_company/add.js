function setUserSourceCompanyAddDatatable(){
...
/**/let deleteDataUrl = "/administration/user_source_company_delete/";
    let serverSide = true;
/**/let apiSource = '/source/api/companies?format=datatables';
/**/let columns = [
        {
            orderable: false,
            searchable: false,
        ...
        {"data" : "name", className:"text-start ps-1 pe-1"},
        {"data" : "formalName", className:"text-start ps-1 pe-1"}
    ];
...
function formSubmitMessageUserSourceCompanyAdd(){
    let ee = elementTag;
    let ei = elementTagId;
    let u = pageUrl;
    console.log(u);
    ...
    $("#form-" + ee +  "-" + ei + "-add-2").submit(function (event) {
      ...
      tableUserSourceCompanies = $("#table-" + elementTagSub + "-" + elementTagId);
      
      for(let i = 0; i < tableUserSourceCompanies.DataTable().rows({selected:true}).data().length; i++){
        formData.push({name: "userSourceCompanies", value: tableUserSourceCompanies.DataTable().rows({selected:true}).data()[i]["id"]});
      };
        ...
        success: function (response, status, xhr) {
            ...
            if (xhr.status === 204) {
                setTimeout(function() {
                    ...
                    $("#table-userSourceCompany-" + elementTagId).DataTable().ajax.reload(function(){
                        htmx.process("#tale-userSourceCompany-" + elementTagId);
                    },false);
...
$(document).ready(function () {
    setUserSourceCompanyAddDatatable();
    //formSubmitMessageSourceCompanyAdd();
});