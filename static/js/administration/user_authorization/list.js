...
function setUserAuthorizationDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/administration/user_authorization_add/";
...
/**/let deleteDataUrl = "/administration/user_authorization_delete/";
    let serverSide = true;
/**/let apiSource = '/administration/api/user_authorizations?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/administration/user_authorization_update/' + row.pk + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "username", className:"double-clickable text-start ps-1 pe-1"},
                    {"data" : "position", className:"double-clickable text-start ps-1 pe-1"}
    ];
  ...
  /**/htmx.ajax('GET', '/administration/user_authorization_update/' + data["pk"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/administration/user_authorization_update/' + data["pk"] + '/');
    });
    ...
    if(deleteDataButton){
        $(deleteDataButtonId).click(function (){
          ...
          for(let i = 0; i < table.DataTable().rows({selected:true}).data().length; i++){
              idList.push(table.DataTable().rows({selected:true}).data()[i]["pk"]);
          };
...
function setNavTabUserAuthorization(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
...
function setNavTabSubUserAuthorization(){
...
function setHTMXUserAuthorization(){
  let ee = elementTag;
  let ei = elementTagId;
...
/**/setUserAuthorizationDataTable();
    setNavTabUserAuthorization();
    setNavTabSubUserAuthorization();
    setHTMXUserAuthorization();
    formSubmitMessage();
