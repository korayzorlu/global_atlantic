...
function setAccessAuthorizationDataTable(){/**/
    ...
    let table = $('#table-' + ef);
/**/let addDataHxGet = "/administration/access_authorization_add/";
...
/**/let deleteDataUrl = "/administration/access_authorization_delete/";
    let serverSide = true;
/**/let apiSource = '/administration/api/access_authorizations?format=datatables';
/**/let columns = [
                    {
                        orderable: false,
                        searchable: false,
                    ...
                    {"data" : "name", className:"double-clickable text-start ps-1 pe-1", render: function (data, type, row, meta)
                        {return '<a hx-get="/administration/access_authorization_update/' + row.id + '/" hx-target="' + addFormBlockSubID + '" hx-swap="afterbegin" hx-push-url="true" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';}
                        //return '<a href="/card/company_update/' + row.id + '/" style="cursor: pointer;text-decoration:underline;">' + data + '</a>';
                    },
                    {"data" : "code", className:"double-clickable text-start ps-1 pe-1"}
    ];
  ...
  /**/htmx.ajax('GET', '/administration/access_authorization_update/' + data["id"] + '/', {target : addFormBlockSubID, swap : "afterbegin", "push-url" : "true"});
      window.history.pushState({}, '', '/administration/access_authorization_update/' + data["id"] + '/');
    });
...
function setNavTabAccessAuthorization(){
  let e = elementTag;
  let ef = elementTag;
  let u = pageUrl;
...
function setNavTabSubAccessAuthorization(){
...
function setHTMXAccessAuthorization(){
  let ee = elementTag;
  let ei = elementTagId;
...
/**/setAccessAuthorizationDataTable();
    setNavTabAccessAuthorization();
    setNavTabSubAccessAuthorization();
    setHTMXAccessAuthorization();
    formSubmitMessage();
