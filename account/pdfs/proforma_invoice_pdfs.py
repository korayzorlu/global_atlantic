...
from card.models import EnginePart, Vessel, Billing, Company,Currency
from sale.models import QuotationPart
from service.models import OfferServiceCard
from source.models import Bank as SourceBank


def proformaInvoicePdf(theRequest, orderConfirmation, proformaInvoice, delivery, activeProject, sourceCompany):
    ...
    if orderConfirmation:
        ...
        quotation = None
    qParts = QuotationPart.objects.filter(quotation = quotation)
    oServiceCards = OfferServiceCard.objects.filter(offer = activeProject)
    parts = ProformaInvoiceItem.objects.filter(invoice = proformaInvoice).order_by("sequency")
    expenses = ProformaInvoiceExpense.objects.filter(invoice = proformaInvoice)
    items = []
    for part in parts:
        if part.quotationPart:
            items.append({"name":part.quotationPart.inquiryPart.requestPart.part.partNo,
        ...
        elif part.offerServiceCard:
            cardDesc = ""
            if part.offerServiceCard.serviceCard.name:
                cardDesc = cardDesc + part.offerServiceCard.serviceCard.name
                if part.offerServiceCard.remark:
                    cardDesc = cardDesc + " - " + part.offerServiceCard.remark
            else:
                if part.offerServiceCard.remark:
                    cardDesc = cardDesc + part.offerServiceCard.remark
                
            items.append({"name":part.offerServiceCard.serviceCard.code,
                        "description" : cardDesc,
                        "quantity" : part.quantity,
                        "unit" : part.offerServiceCard.serviceCard.unit,
                        "unitPrice" : part.unitPrice,
                        "totalPrice" : part.totalPrice
                        })
        elif part.offerPart:
            items.append({"name":part.offerPart.part.partNo,
                        "description" : part.offerPart.part.description,
                        "quantity" : part.quantity,
                        "unit" : part.offerPart.part.unit,
                        "unitPrice" : part.unitPrice,
                        "totalPrice" : part.totalPrice
                        })
        elif part.offerExpense:
            items.append({"name":part.offerExpense.expense.code,
                        "description" : part.offerExpense.expense.name,
                        "quantity" : part.quantity,
                        "unit" : part.offerExpense.expense.unit,
                        "unitPrice" : part.unitPrice,
                        "totalPrice" : part.totalPrice
                        })
    ...
    for serviceCard in oServiceCards:
        partsSubTotal = partsSubTotal + serviceCard.totalPrice
    ...
    vatTotal = (partsSubTotal - discountTotal) * (proformaInvoice.vat / 100)
    ...
    if activeProject:
        if activeProject.discountAmount > 0:
            partsTotalsDict["totalDiscount"] = activeProject.discountAmount
        else:
            partsTotalsDict["totalDiscount"] = partsTotalsDict["totalTotalPrice3"] * (activeProject.discount/100)
    ...
    partsTotalsDict["totalVat"] = (partsTotalsDict["totalTotalPrice3"] - partsTotalsDict["totalDiscount"]) * (proformaInvoice.vat/100)
    ...
    folderPath = os.path.join(os.getcwd(), "media", "docs", str(sourceCompany.id), "account", "proforma_invoice", "documents")
    ...
    #standart ayar
    p = canvas.Canvas(folderPath + "/" + proformaInvoice.proformaInvoiceNo + ".pdf", pagesize = A4)
    ...
    #####sol üst tablo#####
    #company
    vessel = proformaInvoice.vessel
    if vessel:
        vesselName = str(vessel.get_type_display()) + " " + str(vessel.name) + " - IMO No: " + str(vessel.imo)
    else:
        vesselName = ""
        
    if proformaInvoice.billing:
        billingName = proformaInvoice.billing.name
        if proformaInvoice.billing.address:
            billingAddress = proformaInvoice.billing.address
            if proformaInvoice.billing.city:
                billingCity = proformaInvoice.billing.city.name
                billingAddress = billingAddress + " " + billingCity + " /"
            if proformaInvoice.billing.country:
                billingCountry = proformaInvoice.billing.country.international_formal_name
    ...
    if billingAddress:
        ...
        p.drawString(35, current_y, "c/o " + customerName)
    else:
        current_y = h-120
        p.drawString(35, current_y, "c/o " + customerName)
    ...
    if theRequest:
        ...
        projectNo = theRequest.requestNo
        orderDate = orderConfirmation.orderConfirmationDate.strftime("%d.%m.%Y")
        
        if orderConfirmation:
            deliveryType = orderConfirmation.quotation.delivery
        else:
            deliveryType = ""
            
        if delivery:
            deliveryNo = delivery.trackingNo
        else:
            deliveryNo = ""
        
        customerRef = theRequest.customerRef
        payment = orderConfirmation.quotation.payment
        
    if activeProject:
        if activeProject.vessel:
            imo = activeProject.vessel.imo
        else:
            imo = ""
        
        if activeProject.equipment:
            if activeProject.equipment.maker:
                maker = activeProject.equipment.maker
            else:
                maker = ""
        else:
            maker = ""
        ...
        if activeProject.equipment:
            if activeProject.equipment.makerType:
                makerType = activeProject.equipment.makerType
            else:
                makerType = ""
        else:
            makerType = ""
            
        projectNo = activeProject.offerNo
        orderDate = activeProject.offerDate.strftime("%d.%m.%Y")
        deliveryType = activeProject.deliveryMethod
        deliveryNo = ""
        
        customerRef = activeProject.customerRef
        payment = activeProject.paymentType
    ...
    #standart ayar
    if len(items) < 70 and len(items) > 40:
        slice = int(42 - ((banksLength - 10) / 10))
        #slice = 38
    else:
        #slice = 31
        slice = int(35 - ((banksLength - 10) / 10))
    pageNum = 1
    ...
    for i in range(0, len(items), slice):
        ...
        p.setFont('Inter', 6)
        p.drawString((w/2+5)+((w/2-35)/2)+30, h-77, str(proformaInvoice.proformaInvoiceDate.strftime("%d.%m.%Y")))
        #####sağ üst yazılar-end#####
        ...
        #sütun(tablo) 3
        data=[["Açıklama\nDescription"]]
        table = Table(data, colWidths=((w-60)/100)*65, rowHeights=(partsTableRowHeight*2)-5)
        ...
        table = Table(data, colWidths=((w-60)/100)*53, rowHeights=partsTableRowHeight)
        table.setStyle(partsTableStyleDescriptionLeftWhite)
        ...
        #sütun(tablo) 6
        data=[["Birim Fiyatı\nUnit Price"]]
        table = Table(data, colWidths=((w-60)/100)*10, rowHeights=(partsTableRowHeight*2)-5, hAlign="RIGHT")
        table.setStyle(partsTableStyleCenter)
        ...
        for part in partsList:
            ...
            data.append([str(unitPrice3Fixed) + " " + proformaInvoice.currency.code])
        ...
        #sütun(tablo) 7
        data=[["Toplam Fiyat\nTotal Price"]]
        table = Table(data, colWidths=((w-60)/100)*10, rowHeights=(partsTableRowHeight*2)-5)
        table.setStyle(partsTableStyleCenter)
        ...
        for part in partsList:
            ...
            data.append([str(totalPrice3Fixed) + " " + proformaInvoice.currency.code])
        ...
        if pageNum == pageCount:
            ...
            if len(items) == slice or len(items) % slice > 36:
                pageCount = pageCount + 1
            ...
            table.wrapOn(p, 30+table1Length+table3Length+table4Length+table5Length+5-32, 100+banksLength)
            table.drawOn(p, 30+table1Length+table3Length+table4Length+table5Length+5-32, 100+banksLength)
            tableTotalLeftLength = table._colWidths[0]
            ...
            dataSubTotal=[[str(totalTotalPrice3Fixed) + " " + proformaInvoice.currency.code],
                            [str(totalDiscountFixed) + " " + proformaInvoice.currency.code],
                            [str(totalVatFixed) + " " + proformaInvoice.currency.code],
                            [str(totalFinalFixed) + " " + proformaInvoice.currency.code]
                            ]
            table = Table(dataSubTotal, colWidths=((w-60)/100)*13, rowHeights=13)
            table.setStyle(totalRightTableStyleRightTotal)
            
            table.wrapOn(p, 30+table1Length+table3Length+table4Length+table5Length+tableTotalLeftLength+5-32, 100+banksLength)
            table.drawOn(p, 30+table1Length+table3Length+table4Length+table5Length+tableTotalLeftLength+5-32, 100+banksLength)
            tableTotalLeftLength = table._colWidths[0]
